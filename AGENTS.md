...
- This repository is a single Cloudflare Worker that accepts a target page URL in the request path (`GET /<url-encoded-target-url>`), fetches that page, extracts Open Graph and related page signals, and returns JSON.
...
- `src/og-parser.ts` uses Cloudflare `HTMLRewriter` handlers instead of a DOM parser. It maps well-known `og:*` fields to top-level response properties, preserves ordered parsed meta tags in `meta_tags`, captures `<html lang>`, and parses every `script[type="application/ld+json"]` block into `ldJsons`.
